...
class Book:
	...
	def __str__(self):
		return "%s by %s" % (self.title, ", ".join(dune.authors))
	
	def check_out(self):
		if self.available == False:
			raise BookError("That book's already checked out!")
		else:
	...
	def check_in(self):
		if self.available == False:
			...
			raise BookError("That book's not ours!")
	
	def add_author(self, name):
		self.authors.append(name)
		
dune = Book("Dune")
dune.add_author("Frank Herbert")
